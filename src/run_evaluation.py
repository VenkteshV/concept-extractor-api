...
from main.evaluation.embedrank_transformers import CoTagRankPositional,CoTagRankUSE, CoTagRankWindow
...
def evaluation():
    for dataset in ListOfDatasets:
        ...
        for res in results:
            if formatOutput == "df":
                display(res)
                res.to_csv("results-latest-damping_cotag-0"+dataset+".csv")
            else:
                print(res)
